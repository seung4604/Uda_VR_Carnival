...
public class CarnivalCoinToss : MonoBehaviour {
    ...
    void Update() {
        if (coinPickedUp) {
            ...
            if ( (target - curr).magnitude > .01f ) {
                currCoin.transform.position = Vector3.Lerp(curr, target, Time.deltaTime);
            }
    ...
    public void TossCoin() {
        if (coinPickedUp) { 
            ...
            r.isKinematic = false;
            Vector3 targetVel = VRView.transform.position + (VRView.transform.forward);
